...
/*   Updated: 2022/08/05 15:10:18 by jcalon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	num_to_unset(t_separate *list, char	**cmd)
{
	...
	while (cmd[i])
	{
		...
		{
			g_return_code = errmsg("unset: ", cmd[i], ": not a valid identifier");
			return (0);
		}
		if (ft_isalpha(cmd[i][0])
	...
	if (list->begin->env[0] == NULL)
	{
		...
		while (cmds[j])
		{
			if (!ft_isalpha(cmds[j][0]))
				g_return_code = errmsg("unset: ", cmds[j], ": not a valid identifier");	
			j++;
		}
		return (0);
	}
